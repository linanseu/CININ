...
void edge_serve_stealing_thread(void *arg){
#if DATA_REUSE
...
void edges_stealer_edge(uint32_t N, uint32_t M, uint32_t fused_layers, char* network, char* weights, uint32_t edge_id){
   device_ctxt* ctxt = edges_edge_init(N, M, fused_layers, network, weights, edge_id);
...
void edges_victim_edge(uint32_t N, uint32_t M, uint32_t fused_layers, char* network, char* weights, uint32_t edge_id){
   device_ctxt* ctxt = edges_edge_init(N, M, fused_layers, network, weights, edge_id);
   exec_barrier(START_CTRL, TCP, ctxt);
#if DEBUG_TIMING
   start_time = sys_now_in_sec();
   ...
   sys_thread_t t2 = sys_thread_new("send_result_thread", send_result_thread, ctxt, 0, 0);
   sys_thread_t t3 = sys_thread_new("edges_serve_stealing_thread", edges_serve_stealing_thread, ctxt, 0, 0);
