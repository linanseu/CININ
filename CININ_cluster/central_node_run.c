...
#if DEBUG_FLAG
      printf("Results for client %d are all collected in result_gateway, update ready_pool\n", cli_id);
#endif
#if DEBUG_LOG
      char logname[20];
      sprintf(logname, "gateway.log");
      FILE *log = fopen(logname, "a");
      fprintf(log, "Results for client %d, frame %d are all collected in result_gateway, update ready_pool at time %f\n", cli_id, frame_seq, sys_now_in_sec()-start_time);
      fclose(log);
#endif
#if DEBUG_TIMING
      printf("Client %d, frame sequence number %d, all partitions are merged in merge_result_thread\n", cli_id, frame_seq);
      now = sys_now_in_sec();
      /*Total latency*/
...
void gateway(uint32_t N, uint32_t M, uint32_t fused_layers, char* network, char* weights, uint32_t total_edge_number, const char** addr_list){
   device_ctxt* ctxt = gateway_init(N, M, fused_layers, network, weights, total_edge_number, addr_list);
