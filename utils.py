...
def save_checkpoint(state, is_best, path='.', best_acc = 0.0, epoch = 0, filename='checkpoint.pth.tar', save_all=False):
    #filename = os.path.join(path, filename)
    #torch.save(state, filename)
    #if is_best:
    #    filename = 'Best_' + str(best_acc) + '.txt'
    #    filename = os.path.join(path, filename)
    #    torch.save(state, filename)
    #    #shutil.copyfile(filename, os.path.join(path, 'model_best.pth.tar'))
    filename = 'epoch_' + str(epoch) + '_accuracy_' + str(best_acc) + '.pth.tar'
    filename = os.path.join(path, filename)
    torch.save(state, filename)
    #if save_all:
